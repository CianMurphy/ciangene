...
data='/scratch2/vyp-scratch2/cian/UCLex_${release}/allChr_snpStats_ou'
...
echo "
$ldak  --calc-genes-reml $bDir$oDir --bfile $data --pheno $phenotypes --mpheno 1 --ignore-weights YES --minmaf $maf --minvar $minVar --minobs $missing --partition $partition --permute YES --extract $extract --maxmaf $maxmaf  --overlap NO --min-weight 3
" > $oDir$oFile
...
oFile='partition_'$partition'.sh'
echo "
$ldak  --calc-genes-reml $bDir$oDir --bfile $data --pheno $phenotypes --mpheno 1 --ignore-weights YES --minmaf $maf --minvar $minVar --minobs $missing --partition $partition --extract $extract --maxmaf $maxmaf --overlap NO --min-weight 3
" > $oDir$oFile
cd $oDir ; runSh $oFile ; cd .. 
done
...
oFile='partition_'$partition'.sh'
echo "
$ldak --calc-genes-reml $bDir$oDir --bfile $data --pheno $phenotypes --mpheno 1 --ignore-weights YES --minmaf $maf --minvar $minVar --minobs $missing --partition $partition --grm $kinship --extract $extract --maxmaf $maxmaf --overlap NO --min-weight 3
" > $oDir$oFile
cd $oDir ; runSh $oFile ; cd .. 
done 

...
par(mfrow=c(2,2), cex.main = 0.6) 
	...
	for(i in 1:length(files)) 	
	{
		...
		GI <- qq.chisq(-2*log(as.numeric(as.character(file$LRT_P_Perm))), df=2, x.max=30, main = paste(batches[i], "Base", sum(file$Gene_Weight)) ,  pvals=T)
		...
		GI <- qq.chisq(-2*log(as.numeric(as.character(pvals))), df=2, x.max=30, main = paste(batches[i], "Perm", sum(file$Gene_Weight)) ,  pvals=T)
		legend("topleft", , signif(GI[3][[1]]),2) 
		...
		GI <- qq.chisq(-2*log(as.numeric(as.character(file$LRT_P_Perm))), df=2, x.max=30, main = paste(batches[i], "Tech", sum(file$Gene_Weight)) ,  pvals=T)
		legend("topleft", , signif(GI[3][[1]]),2) 

...
noKin <- list.files(iDir, pattern = "no_kin")
techKin <- list.files(iDir, pattern = "Tech_kin")

groups <- gsub(techKin, pattern = "_.*", replacement = "")

annotations <- read.table(paste0(bDir, "annotations.snpStat"), header=T)
